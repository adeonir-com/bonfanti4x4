...
task('pages', () => src(pages.src)
    .pipe(newer(pages.build))
    .pipe(dest(pages.build))
    .pipe(browsersync.stream())
)

task('clean', () => src(clean.build, { read: false })
    .pipe(cleaner())
)
...
task('watchy', () => {
  ...
  watch(pages.src, series('pages'))
})

task(
  'dev',
  parallel('images', 'styles', 'scripts', 'pages')
)

task(
  'default',
  series('dev', parallel('browser-sync', 'watchy'))
)
